...
public class Cribbage extends CardGame {
    ...
    String canonical(ArrayList<Card> cards) {
        Hand h1 = new Hand(deck); // Clone to sort without changing the original hand
        for (Card C : cards) h1.insert(C.getSuit(), C.getRank(), false);
        h1.sort(Hand.SortType.POINTPRIORITY, false);
        return "[" + h1.getCardList().stream().map(this::canonical).collect(Collectors.joining(","))
            + "]";
    }

    class MyCardValues implements Deck.CardValues { // Need to generate a unique
                                                    // value for every card
        public int[] values(Enum suit) { // Returns the value for each card in the suit
            return Stream.of(Rank.values())
                .mapToInt(r -> (((Rank) r).order - 1) * (Suit.values().length) + suit.ordinal())
                .toArray();
        }
    }
    ...
    private void updateScore(int player) {
        ...
        scoreActors[player] =
            new TextActor(String.valueOf(scores[player]), Color.WHITE, bgColor, bigFont);
    ...
    private void deal(Hand pack, Hand[] hands) {
        ...
        // zfc deal log
        for (int i = 0; i < nPlayers; i++) {
            Log.instance().logToFile(String.format("deal,P%d,[", i));
            Log.instance().logToFile(canonical(hands[i]) + "\n");
            //for(int j = 0; j < hands[i].getNumberOfCards(); j++){
            //    Card card = hands[i].get(j);
            //    Log.instance().logToFile(String.format(canonical(card)));
            //    if(j + 1 != hands[i].getNumberOfCards()){
    ...
    private void discardToCrib() {
        ...
        for (IPlayer player : players) {
            ...
            Log.instance().logToFile(String.format("discard,P%d,",player.id));
            Log.instance().logToFile(canonical(cards) + "\n");
    ...
    private void starter(Hand pack) {
        ...
        starter.draw();
        Card dealt = randomCard(pack);
        Log.instance().logToFile(String.format("starter," + canonical(dealt) + "\n"));
        dealt.setVerso(false);
        transfer(dealt, starter);
        if(cardOrder(dealt) == 11){
            scores[1] += 2;
            Log.instance().logToFile(String.format("score,P1,%d,2,starter,%s\n", scores[1], canonical(dealt)));
        }
    }
    ...
    class Segment {
        ...
        void reset(final List<Hand> segments) {
            ...
            segment.setView(
                Cribbage.this, new RowLayout(segmentLocations[segments.size()], segmentWidth));
            segment.draw();
        ...
        void addCard(Card card, int player){
            lastPlayer = player;
            transfer(card, segment);
            score += cardValue(card);
            ArrayList<Rule> validRules = new ScoreSegment(segment).getValidRules(deck);
            //System.out.println(validRules.size());
            for(Rule rule: validRules){
                scores[lastPlayer] += rule.toScore();
                updateScore(lastPlayer);
                Log.instance().logToFile(String.format("score,P%d,%d,%d,%s\n", lastPlayer, scores[lastPlayer], rule.toScore(), rule.toString()));
            }
        }
    }
    ...
    private void play() {
        ...
        while (!(players[0].emptyHand() && players[1].emptyHand())) {
            ...
            if (nextCard == null) {
                if (s.go) {
                    ...
                    Log.instance().logToFile(String.format("score,P%d,%d,%d,%s\n", currentPlayer, scores[currentPlayer], 1, "go"));
                }
                currentPlayer = (currentPlayer + 1) % 2;
            } else {
                Log.instance().logToFile(String.format("play,P%d,%d,%s\n", currentPlayer, s.score, canonical(nextCard)));
                s.addCard(nextCard, currentPlayer);
                //score += cardValue(nextCard);
        ...
        if(s.score != 0){
            ...
            Log.instance().logToFile(String.format("score,P%d,%d,%d,%s\n", currentPlayer^1, scores[currentPlayer^1], 1, "go"));
        }
    }

    void showOne(Hand originalHand, int playerId){
        Log.instance().logToFile(String.format("show,P%d,%s+%s\n", playerId, canonical(starter.get(0)), canonical(originalHand)));
        ArrayList<Rule> validRules = Rule.getValidShowRules(originalHand, starter.get(0), deck);
        for(Rule rule: validRules){
            scores[playerId] += rule.toScore();
            updateScore(playerId);
            Log.instance().logToFile(String.format("score,P%d,%d,%d,%s,%s\n", playerId, scores[playerId], rule.toScore(), rule.toString(), canonical(rule.result)));
        }
    }

    void showHandsCrib() {
        showOne(players[0].originalHand, 0);
        showOne(players[1].originalHand, 1);
        showOne(crib, 1);
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException,
        ...
        Log.instance().logToFile(String.format("seed,%d\n", SEED));

        // Control Player Types
        Class<?> clazz;
        clazz = Class.forName(cribbageProperties.getProperty("Player0"));
        Log.instance().logToFile(String.format(cribbageProperties.getProperty("Player0")) + ",P0\n");
        players[0] = (IPlayer) clazz.getConstructor().newInstance();
        clazz = Class.forName(cribbageProperties.getProperty("Player1"));
        Log.instance().logToFile(String.format(cribbageProperties.getProperty("Player1")) + ",P1\n");
